...
if (
    ...
    $validRoles = ['Client', 'Farmer', 'Admin']; // Define valid roles

        if (!in_array($_POST["role"], $validRoles, true)) {
            echo "<script>alert('Invalid role selected. Role must be Client, Farmer, or Admin.');</script>";
        } else {
            $valid = 1; // Role is valid
        }
...
if ($valid == 1) {
    ...
    $updatedRole = $_POST['role']; // Capture the selected role as integer
...
<head>
    ...
    <script>
        ...
        function validateForm(event) {
            ...
            const validRoles = ["Client", "Farmer", "Admin"]; // ENUM values
