...
// Requête pour récupérer toutes les dates des événements
$query = $db->query("SELECT date FROM events");

// Tableau pour stocker le nombre d'événements par mois
$eventsPerMonth = array_fill(0, 12, 0);

// Remplir le tableau avec les résultats de la requête
while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
    $eventMonth = (int) date("n", strtotime($row['date'])) - 1; // 0-indexed month
    $eventsPerMonth[$eventMonth]++;
}
...
<head>
    ...
    <style>
        ...
        .modal {
            ...
            z-index: 1;
            left: 20%;
            /* Déplace la modale 10% vers la droite de l'écran */
            top: 0;
            width: 80%;
...
<header id="header" class="header fixed-top d-flex align-items-center">
    ...
    <div class="search-bar">
        <form class="search-form d-flex align-items-center" method="POST" action="#">
            <input type="text" name="query" id="searchQuery" placeholder="Looking for something?"
...
<main id="main" class="main">
    ...
    <div style="width: 70%; margin: auto;">
        <canvas id="eventsByMonthBarChart"></canvas>
    </div>
    <div style="width: 70%; margin: auto;">
    ...
    <script>
        ...
        document.querySelector('.search-form').addEventListener('submit', function (e) {
            ...
            rows.forEach(row => {
                ...
                // Check if the search query matches any cell (ID, Title, Date, Longitude, Latitude)
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            // Préparation des données dynamiques depuis PHP
            <?php
            echo "const months = " . json_encode($months) . ";";
            echo "const eventsPerMonth = " . json_encode($eventsPerMonth) . ";";
            ?>
            ...
            new Chart(ctx1, {
                ...
                data: {
                    labels: months, // Labels pour l'axe x (mois)
                    datasets: [{
                        label: "Événements par mois", // Titre du graphique
                        data: eventsPerMonth, // Données pour l'axe y
                ...
                options: {
                    ...
                    scales: {
                        y: {
                            beginAtZero: true // Commencer à 0 sur l'axe y
                        }
                    }
                }
            });
            const ctxBar = document.getElementById("eventsByMonthBarChart").getContext("2d");
            new Chart(ctxBar, {
                type: "bar",
                data: {
                    labels: months,
            ...
            const ctxPie = document.getElementById("eventsPieChart").getContext("2d");
            new Chart(ctxPie, {
                type: "pie",
                data: {
                    labels: months,
